...
LDREF="${LDREF:-EAS_phase3_plink1_nodup}"
PHENO="${PHENO:-SCZ.txt}"
...
RUN_PLINK_ROUND1="${RUN_PLINK_ROUND1:-false}"
...
if [[ ! -f "${BASE}" ]]; then echo "Base summary statistics not found: ${BASE}" >&2; exit 1; fi
if [[ ! -f "${TARGET}.bed" || ! -f "${TARGET}.bim" || ! -f "${TARGET}.fam" ]]; then echo "Target PLINK files not found: ${TARGET}.bed/.bim/.fam" >&2; exit 1; fi
if [[ ! -f "${LDREF}.bed" || ! -f "${LDREF}.bim" || ! -f "${LDREF}.fam" ]]; then echo "LD reference PLINK files not found: ${LDREF}.bed/.bim/.fam" >&2; exit 1; fi
if [[ ! -f "${PHENO}" ]]; then echo "Phenotype file not found: ${PHENO}" >&2; exit 1; fi
...
if [[ -f "${COV}" ]]; then
  prsice_args+=(--cov "${COV}" --cov-col PC1,PC2,PC3,PC4,PC5,PC6,PC7,PC8,PC9,PC10)
else
  echo "Covariate file not found: ${COV}; running without --cov/--cov-col." >&2
fi

if [[ "${EFFECT_IS_OR}" == "true" ]]; then prsice_args+=(--or); else prsice_args+=(--beta); fi
if [[ "${RUN_PLINK_ROUND1}" == "true" ]]; then prsice_args+=(--extract "${ROUND1_SNPS}"); fi
if [[ "${ALL_SCORE}" == "true" ]]; then prsice_args+=(--all-score); fi
if [[ "${PERM}" != "0" ]]; then prsice_args+=(--perm "${PERM}"); fi
